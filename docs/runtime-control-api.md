...
codex-ui-ctl events.subscribe --since-seq 12 --timeout-ms 30000 --pretty
...
| `runtime.snapshot` | 返回当前 panes、真实退出码、状态序号和生命周期事件。 |
| `runtime.orchestrate` | 按强类型步骤一次提交新标签、聚焦、分屏、Prompt、命令和 Agent 启动。 |
| `window.create` / `window.focus` | 复用或聚焦当前窗口。 |
...
| `events.subscribe` | 长轮询等待新的生命周期事件；超时返回空数组，不会伪造事件。 | 
