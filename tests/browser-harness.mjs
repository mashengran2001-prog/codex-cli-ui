...
export async function installMockBridge(page) {
  await page.addInitScript(() => {
    ...
    window.codex = {
      ...
      listImportedFonts: async () => window.__mock.importedFonts || [],
      importFont: async () => window.__mock.importFontResult || { ok: false, error: "canceled" },
      deleteImportedFont: async (fileName) => { const list = (window.__mock.importedFonts || []).filter((font) => font.fileName !== fileName); window.__mock.importedFonts = list; return true; },
