  ...
  const renderPaneNode = (node: PaneNode): ReactNode => {
    ...
    return (
      ...
      >
        ...
        <TerminalPane session={session} theme={settings.theme} cursorStyle={settings.cursorStyle} cursorBlink={settings.cursorBlink} fontFamily={settings.fontFamily} cellWidth={settings.cellWidth} backgroundOverride={settings.backgroundColor} bellFlash={bellFlash.has(session.id)} copyOnSelect={settings.copyOnSelect} renderTerminalMath={settings.renderTerminalMath} builtinBoxDrawing={settings.builtinBoxDrawing} active={isActive && terminalView && windowFocused} onFocus={() => { setActiveSessionId(session.id); setUnread((current) => { const next = new Set(current); next.delete(session.id); return next; }); }} onTerminalReady={(id, terminal) => { if (terminal) terminalInstancesRef.current.set(id, terminal); else terminalInstancesRef.current.delete(id); }} onError={onError} onBroadcast={(data) => broadcastInput(session.id, data)} />
